...
function buildModeRanks(candidate, currentProduct, classification, preferences) {
  ...
  return {
    ...preview,
    healthierRank: preview.score + candidate.confidence * 12 + (sameSubcategory ? 12 : sameCategory ? 6 : 0),
    betterValueRank: preview.score * 0.7 + candidate.confidence * 8 + Math.max(0, 18 - Math.min(18, priceDelta)),
    closerMatchRank: preview.score + candidate.confidence * 10 + (sameSubcategory ? 14 : sameCategory ? 8 : 0) + (preview.preferenceFit || 0)
  };
}
...
export function buildAlternatives({ currentProduct, classification, preferences, history = [], relatedProducts = [], enrichmentCandidates = [] }) {
  ...
  const filtered = uniqueBy(rawCandidates, (candidate) => candidate.url || candidate.title.toLowerCase())
    ...
    .filter((candidate) => candidate.category === classification.category || candidate.sourceType === "history");
