...
function matchedIngredientNames(ingredients, matchers) {
  return [...new Set(
    ingredients
      .filter((item) => matchers.some((matcher) => item.canonicalName.includes(matcher)))
...
export function scoreSkincare(product, preferences) {
  ...
  for (const benefit of SKINCARE_POSITIVE_INGREDIENTS) {
    const matches = matchedIngredientNames(ingredients, [benefit.key]);
    if (matches.length) {
      score += benefit.score;
      benefits.push({ title: benefit.label, detail: benefit.reason, matchedIngredients: matches });
      pushComponent(components, benefit.label, `+${benefit.score}`, benefit.reason);
    }
  }

  for (const rule of SKINCARE_FLAG_INGREDIENTS) {
    const matches = matchedIngredientNames(ingredients, rule.keys);
    if (matches.length) {
      ...
      score -= penalty;
      flags.push({ title: rule.label, severity: rule.severity, effect: `-${penalty}`, detail: rule.reason, matchedIngredients: matches });
...
export function scoreFood(product, preferences) {
  ...
  if (dyeHits.length) {
    ...
    flags.push({
      ...
      matchedIngredients: matchedIngredientNames(ingredients, dyeHits)
    });
  ...
  if (sweetenerHits.length) {
    ...
    flags.push({
      ...
      matchedIngredients: matchedIngredientNames(ingredients, sweetenerHits)
    });
  }
  ...
  if (additiveHits.length >= 2) {
    ...
    flags.push({
      ...
      matchedIngredients: matchedIngredientNames(ingredients, additiveHits)
    });
    ...
    benefits.push({
      ...
      matchedIngredients: ingredients.slice(0, 4).map((item) => item.displayName)
    });
  }
...
export function scoreSupplements(product, preferences) {
  ...
  if (fillerHits.length) {
    ...
    flags.push({
      title: "Filler load",
      severity: fillerHits.length > 1 ? "medium" : "low",
      effect: `-${penalty}`,
      detail: "Multiple filler-style ingredients add some caution for shoppers seeking a simpler label.",
      matchedIngredients: matchedIngredientNames(ingredients, fillerHits)
    });
  }
  ...
  if (sweetenerHits.length) {
    score -= 5;
    flags.push({
      title: "Sweetener burden",
      severity: "medium",
      effect: "-5",
      detail: "Sweeteners are often avoided by shoppers who want a simpler supplement profile.",
      matchedIngredients: matchedIngredientNames(ingredients, sweetenerHits)
    });
  }
...
export function scoreEcoCleaning(product, preferences) {
  ...
  if (fragranceHits.length) {
    ...
    score -= penalty;
    flags.push({
      title: "Fragrance burden",
      severity: "medium",
      effect: `-${penalty}`,
      detail: "Added fragrance and fragrance allergens are commonly avoided by shoppers seeking gentler cleaning products.",
      matchedIngredients: matchedIngredientNames(ingredients, ECO_FRAGRANCE_TERMS)
    });
  }
  ...
  if (harshHits.length) {
    ...
    score -= penalty;
    flags.push({
      title: "Harsh chemistry detected",
      severity: "high",
      effect: `-${penalty}`,
      detail: "The formula appears to include harsher cleaning agents, which may not fit gentler-cleaning preferences.",
      matchedIngredients: matchedIngredientNames(ingredients, harshHits)
    });
  }
