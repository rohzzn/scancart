...
async function loadLocalDevConfig() {
  ...
  localDevConfigPromise = (async () => {
    try {
      const [envConfig, jsonConfig] = await Promise.all([
        loadEnvLocalConfig(),
        loadJsonLocalConfig()
      ]);
      return { ...jsonConfig, ...envConfig };
    } catch (error) {
      return {};
    }
  })();
...
async function loadEnvLocalConfig() {
  try {
    ...
    if (entries.GEMINI_API_KEY) {
      mapped.geminiApiKey = entries.GEMINI_API_KEY;
    }
    if (entries.USDA_API_KEY) {
      mapped.usdaApiKey = entries.USDA_API_KEY;
    }
    if (entries.OPEN_FOOD_FACTS_BASE_URL) {
      mapped.openFoodFactsBaseUrl = entries.OPEN_FOOD_FACTS_BASE_URL;
    }
    if (entries.OPEN_BEAUTY_FACTS_BASE_URL) {
      mapped.openBeautyFactsBaseUrl = entries.OPEN_BEAUTY_FACTS_BASE_URL;
    }
    if (entries.NIH_DSLD_API_BASE_URL) {
      mapped.nihDsldApiBaseUrl = entries.NIH_DSLD_API_BASE_URL;
    }
    if (entries.USDA_API_BASE_URL) {
      mapped.usdaApiBaseUrl = entries.USDA_API_BASE_URL;
    }
...
async function loadJsonLocalConfig() {
  try {
...
export async function getSettings() {
  ...
  // Only apply saved settings where the value is non-empty, so .env.local keys
  // are never silently overridden by a blank saved value.
  const savedSettings = result.settings || {};
  const nonEmptySaved = Object.fromEntries(
    Object.entries(savedSettings).filter(([, v]) => v !== "" && v !== null && v !== undefined)
  );

  return { ...DEFAULT_SETTINGS, ...nonEmptySaved, ...localDevConfig };
}
